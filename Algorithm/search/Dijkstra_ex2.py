...
# 하지만 X라는 도시에서 Y라는 도시로 전보를 보내고자 한다면, 도시 X에서 Y로 향하는 통로가 설치되어 있어야한다. 예를 들어 X에서 Y로 향하는 통로는 있지만,
...
# 최대한많이 퍼져나갈 것이다. 각 도시의 번호와 통로가 설치되어 있는 정보가 주어졌을 때, 도시 C에서 보낸 메시지를 받게 되는 도시의 개수는 총 몇 개 이며 도시들이
...
# 2. 둘째 줄붜 M + 1번쨰 줄에 걸쳐서 통로에 대한 정보 X,Y,Z가 주어진다. 이는 특정 도시 X에서 다른 특정 도시 Y로 이어지는 통로가 있으며, 메시지가 전달되는
#  시간이 Z라는 의미다. (1 <= X, Y <= N, 1 <= Z <= 1,000)

# 출력
# 1. 첫째 줄에 도시 C에서 보낸 메시지를 받는 도시의 총 개수와 걸리는 시간을 공백으로 구분하여 출력한다.
